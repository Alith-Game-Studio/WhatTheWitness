...
func __shrink_shape(shape, depth, scale, center):
	var result = []
	for i in range(len(shape)):
		var p0 = shape[i - 1] if i >= 1 else shape[len(shape) - 1]
		var p1 = shape[i]
		var p2 = shape[i + 1] if i + 1 < len(shape) else shape[0]
		result.append(__shrink_corner(p0, p1, p2, depth) * scale + center)
	return result
...
func draw_foreground(canvas: Visualizer.PuzzleCanvas, owner, owner_type: int, puzzle: Graph.Puzzle):
	...
	if (is_hollow):
		for shape in shapes:
			var hollow_shape = __shrink_shape(shape, margin_size, scale, owner.center)
			if(!hollow_shape.empty()):
				hollow_shape.append(hollow_shape[0])
			var inner_shape = __shrink_shape(shape, border_size, scale, owner.center)
		...
		for shape in shapes:
			canvas.add_polygon(__shrink_shape(shape, margin_size, scale, owner.center), color)
